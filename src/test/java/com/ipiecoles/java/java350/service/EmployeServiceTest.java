...
public class EmployeServiceTest {
	...
	public void testFindByImmatNotFound() throws EntityExistsException, EmployeException {
		...
		// then call real method is for calling the real method an get the value on this
		// call instead of giving a value

		// findByMatricule => null
		Mockito.when(employeRepository.findByMatricule("C00346")).thenReturn(null); // l'id après celui qu'on donné dans
																					// la prmière parite (le suivant)
		...
		ArgumentCaptor<Employe> employeCaptor = ArgumentCaptor.forClass(Employe.class); // on peut prendre la valeur ou
																						// l'argument d'une méthode
		Mockito.verify(employeRepository, Mockito.times(1)).save(employeCaptor.capture()); // ici on regrade: Est ce que
																							// la méthode sava de
																							// l'employeRepository a été
																							// appelée au moin une seule
																							// fois?
		// pour la calsse Employé en mettant en paramètre la variable employeCaptor de
		// la classe Employe// le pramètre time n'est pas obligatoire // mokito.never()
		// est pour vérifier est ce qu'on est dans un bloc d'if ou ect
...
//        Assertions.assertThat(employe).isEqualTo(employeVerif);
	...
	public void testLimteMatricule() throws EntityExistsException, EmployeException {
		...
		// findLastMatrcule => 00345 /null
		Mockito.when(employeRepository.findLastMatricule()).thenReturn("99999"); // when this meéthod est applé retourne
																					// comme value 99999
		// Mockito.when(employeRepository.findLastMatricule()).thenCallRealMethod();
		// then call real method is for calling the real method an get the value on this
		// call instead of giving a value
